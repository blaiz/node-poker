...
function updateTurn (table) {
    if (table.game) {
        ...
        } while (table.players[table.game.turn].justAdded === true);
    }
}
...
function prepareNextBettingRound(table, numOfCards) {
    ...
    // After a round is over, the min raise goes back to 0 (no one has raised yet)
    table.game.lastRaise = 0;
...
Table.prototype.NewRound = function() {
    ...
    this.game.lastRaise = this.bigBlind; // In "Deal" round, the big blind is considered the first bet
};
...
Player.prototype.Bet = function(bet) {
    ...
    // e.g. in the flop round, the minimum amount that can be bet is the big blind, even thought the min raise is 0
        ...
        throw new InvalidActionError("Bet or raise can't be for an amount less than the big blind.")
    }
    ...
    this.table.game.lastRaise = bet + this.table.game.bets[this.playerID] - getMaxBet(this.table.game.bets);
