...
Table.prototype.StartGame = function () {
    ...
    if (!this.game && this.players.length >= this.minPlayers) {
        this.game = new Game();
        this.events.push({time: Date.now(), event: 'Start Game'});
        this.NewRound();
    }
};
...
Table.prototype.AddEvent = function (event) {
    this.events.push({time: Date.now(), event: event});
};
...
Table.prototype.NewRound = function() {
    ...
    this.events.push({time: Date.now(), event: 'Dealing hold cards'});
    ...
    //  For heads up, the dealer is the small blind, and acts first
    if (this.players.length === 2) {
        this.game.smallBlind = 0;
        this.game.bigBlind = 1;
        this.game.turn = 0;
    }
    ...
    this.events.push({time: Date.now(), event: '<strong>' + this.players[this.game.smallBlind].playerName + '</strong> posts small blind of ' + this.smallBlind });
    this.players[this.game.bigBlind].chips -= this.bigBlind;
    this.events.push({time: Date.now(), event: '<strong>' + this.players[this.game.bigBlind].playerName + '</strong> posts big blind of ' + this.bigBlind });
